...
<body>
    ...
    <div class="page-container">
        <div class="text-c">
            <form class="Huiform" method="get" action="{{route('view/fangattr-list')}}">
                <input type="text" class="input-text" style="width:250px" placeholder="规格名称" id="h" name="h" value="{{$h}}">
        ...
        <table class="table table-border table-bordered table-bg">
            ...
            <tbody>
                @foreach ($fangattrs as $val)
                <tr class="text-c">
                    <td><input type="checkbox" class="checked" value="{{$val['id']}}" name=""></td>
                    <td>{{$val['id']}}</td>
                    <td style="text-align: left; padding-left: 2vw;">{{$val['html']}}{{$val['name']}}</td>
                    <td><img height="40px" src="/icon/{{$val['icon']}}" alt=""></td>
