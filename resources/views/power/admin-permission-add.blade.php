		...
		<form method="post" class="form form-horizontal" id="form-admin-role-add">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="@if (!empty($data)) {{$data['power_name']}}@endif" placeholder="" id="roleName" name="power_name">
				</div>
			</div>
			<div class="row cl">
                    ...
                    <span class="hui-select-box">
                        <select class="hui-select" size="1" name="pid">
							...
							@foreach ($powers as $power)
								@if ($data['pid'] == $power['id'])
									<option value="{{$power->id}}" selected>{{$power->html}}{{$power->power_name}}</option>
								@else 
									<option value="{{$power->id}}">{{$power->html}}{{$power->power_name}}</option>
								@endif
                            @endforeach
                        </select>
                    </span>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="@if (!empty($data)) {{$data['route_name']}}@endif" placeholder="" id="route_name" name="route_name">
				</div>
			</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"></label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="checkbox" id="is_show" name="is_show" @if (!empty($data)) @if ($data['is_show'] == "1") checked @endif @endif>
			...
			$("#form-admin-role-add").validate({
				...
				submitHandler: function (form) {
					$(form).ajaxSubmit({
						type: 'POST',
						@if (!empty($data))
						url: '{{route("power/update")}}',
						@else
						url: '{{route("power/create")}}',
						@endif
						async: false,
